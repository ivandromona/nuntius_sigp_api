...
import ao.adnlogico.nuntius.multitenant.tenant.model_assembler.StepModelAssembler;
import ao.adnlogico.nuntius.multitenant.tenant.repository.StepRepository;
...
//    stepService service;
    ...
    @RequestAuthorization
    @GetMapping("/step/{id}")
    public EntityModel<Step> findById(@PathVariable Long id)
    {
    ...
    @RequestAuthorization
    @PutMapping("/step/{id}")
    public ResponseEntity<?> update(@RequestBody Step newstep, @PathVariable Long id)
    {
